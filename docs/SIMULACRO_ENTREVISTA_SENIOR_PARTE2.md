...
### Respuesta:
```python
dummy = DummyRegressor(strategy="median")            # Predice siempre la mediana.
...
### Respuesta:
```python
mape = np.mean(np.abs((y_true - y_pred) / (y_true + 1e-8))) * 100  # 1e-8 evita /0.
```
...
### Respuesta:
```python
# Opción 1: Path versioning                       # Más explícito y cacheable.
@app.post("/v1/predict")
@app.post("/v2/predict")
...
**Carga con override**:
```python
config = BankChurnConfig.from_yaml("configs/config.yaml")  # Config base.

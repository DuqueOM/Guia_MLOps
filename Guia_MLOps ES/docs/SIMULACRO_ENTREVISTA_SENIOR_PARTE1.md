...
```python
class ResampleClassifier(BaseEstimator, ClassifierMixin):  # Wrapper para resampling.
...
```python
class FeatureEngineer(BaseEstimator, TransformerMixin):  # Transformer custom.
...
**Pipeline integrado**:
```python
pipe = Pipeline([("features", fe), ("pre", pre), ("model", model)])  # Pipeline completo.
...
### Respuesta:
```python
cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)  # Mantiene proporción de clases en cada fold.
```
...
### Respuesta:
```python
LogisticRegression(C=0.1, solver="liblinear")        # C=0.1: regularización fuerte (C=1/λ).
```
...
### Respuesta:
```python
for fold, (train_idx, val_idx) in enumerate(cv.split(X_train, y_train), 1):  # Itera folds.
...
### Respuesta:
```python
pipeline = Pipeline([("preprocess", pre), ("clf", clf)])  # Pipeline completo.
joblib.dump(pipeline, "model.joblib")            # Un solo archivo.
```

**Deployment simplificado**:
```python
pipe = joblib.load("model.joblib")               # Carga todo.
...
### Respuesta:
```python
@app.get("/health")                              # Endpoint para health checks.
...
### Respuesta:
```python
@contextlib.asynccontextmanager                  # Context manager asíncrono.
...
### Respuesta:
```python
@app.post("/predict_batch")                      # Endpoint para batch.
...
### Respuesta:
```python
@app.get("/metrics")                             # Endpoint de métricas operacionales.
...
### Respuesta:
```python
class CustomerData(BaseModel):                   # Schema de entrada.
...
### Respuesta:
```python
class PredictionResponse(BaseModel):             # Schema de respuesta.
...
### Respuesta:
```python
class ResampleClassifier(BaseEstimator, ClassifierMixin):  # Compatible con sklearn.
...
### Respuesta:
```python
def test_full_training_workflow(config, sample_data, tmp_path):  # E2E test.
    trainer = ChurnTrainer(config)
...
### Respuesta:
```python
pipe = Pipeline([                                # Pipeline de 3 pasos.
...
### Respuesta:
```python
for _ in range(200):                               # 200 iteraciones bootstrap.
...
### Respuesta:
```python
df_sorted = df.sort_values("model_year")           # Ordena por tiempo.
...
### Respuesta:
```python
@st.cache_data                                     # Cache para datos serializables.
def load_data():
